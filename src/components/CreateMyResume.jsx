...
const CreateMyResume = () => {
  ...
  return (
    <div className="my-10 px-10">
      <p className="p-2 pb-5 text-sm font-medium tracking-wider text-center uppercase">
        Create Your Resume
      </p>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-5 lg:gap-16">
        ...
        <div className="w-full lg:w-1/2">
          <div className="relative w-full h-40 md:h-60 lg:min-h-80 rounded-lg overflow-hidden shadow-lg">
            <iframe
              className="w-full h-full rounded-lg cursor-pointer" // Added cursor-pointer
        ...
        <div className="w-full lg:w-1/2">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {tags.map((tag) => (
              <span
                key={tag.id}
                className="px-4 py-2 text-sm font-medium text-[#20252D] bg-[#20252D]/20 hover:bg-[#20252D]/10 rounded-full cursor-pointer transition duration-300"
              >
                {tag.name}
              </span>
            ))}
