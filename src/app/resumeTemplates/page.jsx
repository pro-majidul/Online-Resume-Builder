...
const ResumeTemplates = () => {
  ...
  const categories = [
    {
      ...
      description: 'Sleek designs with contemporary layouts'
    },
    {
      name: 'Creative',
      icon: '/icon/creative.png',
      description: 'Unique styles for standout applications'
    },
    {
      name: 'Minimalist',
      icon: '/icon/compact-layout.png',
      description: 'Clean, focused, and distraction-free'
    },
    {
      name: 'Professional',
      icon: '/icon/professional.png',
      description: 'Timeless formats for corporate roles'
    }
  ]
  ...
  return (
    <div
      className={`min-h-screen font-sans ${bgColor} ${textColor} transition-colors duration-300`}
    >
      {/* Hero Section */}
      <ParallaxHeader
      ...
      <section className='mx-auto px-4 sm:px-6 py-20 max-w-7xl'>
        <div className='mb-16 text-center'>
          ...
          >
            Select the perfect style that aligns with your industry and
            personality
          </motion.p>
        </div>
        ...
        <div className='gap-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4'>
          {categories.map((category, index) => (
            ...
            >
              <div
                className={`w-16 h-16 rounded-full ${secondaryColor} flex items-center justify-center mb-4`}
              >
                <img
                  src={category.icon}
                  alt={category.name}
        ...
        <div className='mx-auto px-4 sm:px-6 max-w-7xl'>
          <div className='mb-16 text-center'>
            ...
            <motion.p
              ...
              className={`mx-auto max-w-2xl text-xl ${accentColor}`}
            >
              Professionally designed templates optimized for applicant tracking
              systems
            </motion.p>
          </div>
          ...
          <div className='gap-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
            {templates.map((template, index) => (
              ...
              >
                ...
                {/* Overlay Content */}
                <div
                  className={`absolute inset-0 flex flex-col justify-end p-6 bg-gradient-to-t from-black/90 via-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                >
                  <div className='mb-2'>
                    <span
                  ...
                  <ul className='mb-4'>
                    {template.features.map((feature, i) => (
                      <li
                        key={i}
                        className='flex items-center mb-1 text-gray-300 text-sm'
                      >
                        <svg
                          className='mr-2 w-3 h-3 text-yellow-400'
                          fill='currentColor'
                          viewBox='0 0 20 20'
                        >
                          <path
          ...
          <div className='mt-16 text-center'>
            ...
            >
              <CustomButton
                ...
                } px-8 py-3 rounded-lg font-semibold shadow-lg transition-all duration-300 inline-flex items-center`}
                icon={
                  <svg
                    className='ml-2 w-5 h-5'
                    fill='none'
      ...
      <section className='mx-auto px-4 sm:px-6 py-20 max-w-7xl'>
        ...
        <div className='gap-8 grid grid-cols-1 md:grid-cols-3'>
          {[
            {
              quote:
                'The modern template helped me stand out in a competitive tech market. I received 3 interview requests within a week!',
              name: 'Sarah Johnson',
            ...
            {
              quote:
                'As a recent graduate, the professional template gave me credibility. I landed my first job at a Fortune 500 company.',
              name: 'Michael Chen',
            ...
            {
              quote:
                'The creative template perfectly showcased my portfolio. I doubled my freelance client base in two months.',
            ...
            >
              ...
              <p className={`text-lg italic mb-6 ${textColor}`}>
                "{testimonial.quote}"
              </p>
      ...
      {isPreviewOpen && selectedTemplate && (
        ...
        >
          <motion.div
            ...
            animate={{ scale: 1, y: 0 }}
            className={`relative max-w-4xl w-full ${cardBg} rounded-2xl shadow-2xl overflow-hidden`}
            onClick={e => e.stopPropagation()}
          >
            <div
              className={`p-6 flex justify-between items-center border-b ${borderColor}`}
            >
              <h3 className='font-bold text-xl'>{selectedTemplate.name}</h3>
              <button
                onClick={() => setIsPreviewOpen(false)}
                className='hover:bg-gray-200/50 dark:hover:bg-gray-700/50 p-2 rounded-full'
              >
                <XIcon className='w-5 h-5' />
              </button>
            </div>

            <div className='gap-8 grid grid-cols-1 lg:grid-cols-2 p-6'>
              <div
                className={`relative aspect-[1/1.414] shadow-lg border ${borderColor}`}
              >
                <Image
              ...
              <div>
                ...
                <ul className='space-y-2 mb-6'>
                  {selectedTemplate.features.map((feature, i) => (
                    <li key={i} className='flex items-start'>
                      <CheckCircleIcon className='flex-shrink-0 mt-0.5 mr-2 w-5 h-5 text-green-500' />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className='space-y-4'>
                  <CustomButton
                    ...
                    className='bg-gradient-to-r from-yellow-400 hover:from-yellow-500 to-yellow-500 hover:to-yellow-600 shadow-md py-3 rounded-lg w-full font-bold text-gray-900'
                  />
                  <CustomButton
                    title='Preview Full Screen'
                    variant='outline'
                    className={`w-full border ${borderColor} hover:bg-gray-100/50 dark:hover:bg-gray-700/50`}
                  />
                </div>
              </div>
            </div>
